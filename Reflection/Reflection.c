...
//  printf("%f\n",rtMat[count]);
		...
		GLfloat left_light_position[] = { 10.0, 0.0, -10.0, 1.0 }, right_light_position[] = { -10.0, 0.0, -10.0, 1.0 }, front_light_position[] = { 0.0, 0.0, -10.0, 1.0 };
		...
		//	printf("Total Memory %ld",noOfVertices*sizeof(vertices)+noOfFaces*sizeof(polygons)+noOfVertices*sizeof(normals));
			...
			for (i = 0; i < noOfVertices; i++) {
				normals[i].normal[0]/=(double)normals[i].count;
				normals[i].normal[1]/=(double)normals[i].count;
				normals[i].normal[2]/=(double)normals[i].count;
			}
		...
		static void Render() {
			...
			if (usePerVertexNormals) {
				for (i = 0; i < noOfFaces; i++) {
					...
					glNormal3dv(normals[v1].normal);

					glVertex3dv(vertices[v1].coord);

					glNormal3dv(normals[v2].normal);

					glVertex3dv(vertices[v2].coord);

					glNormal3dv(normals[v3].normal);
				...
				for (i = 0; i < noOfFaces; i++) {
					...
					glNormal3dv(normalsPerFragment[i].normal);
